<?php
    ...
    while ( $r->have_posts() ) :
        ...
        // format output
        if ( $post_counter == 1 ) {
            $att_img_src                    = wp_get_attachment_image_src( get_post_thumbnail_id(), array(1170,400));
            $post_article->article_thumb    = ( has_post_thumbnail() ) ? $att_img_src[0] : MipTheme_Global::mp_set_dummy_img( '1170x400' );
        
            echo '<div class="row">';
            echo '<div class="col-sm-12">';
            echo $post_article->formatArticleStyle8(1170,400);
            echo '</div>';
        } else {
