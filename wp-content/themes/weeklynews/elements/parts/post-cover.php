...
<?php
    elseif ( has_post_thumbnail() ) :
        // check for thumbnail
        // limit featured image height
        
        $featured_image             = new MipTheme_Image();
        $featured_image_attr        = $featured_image->get_image_loop_page_3($featured_image_height_limit);
...
<!-- start:cover image -->
<div class="head-image thumb-wrap relative cat-<?php if ($category[0]) echo esc_attr($category[0]->cat_ID); ?>">
    <div class="relative">

        <?php
            if ( $format == 'image' ) {
                $att_img_zoom   = wp_get_attachment_image_src( get_post_thumbnail_id(), 'large');
                $img_zoom       = $att_img_zoom[0];
                echo '<div class="zoom-photo"><a href="'. esc_url($img_zoom) .'">'. get_the_post_thumbnail( $post->ID, $curr_post_img, $img_attr ) .'<div class="zoomix"><i class="fa fa-search"></i></div></a></div>';
            } else {
                echo get_the_post_thumbnail( $post->ID, $curr_post_img, $img_attr );
            }
