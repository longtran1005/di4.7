 ...
 * @since 0.4
 */
if ( ! function_exists( 'breadcrumbs_plus_get_items' ) ) {
	function breadcrumbs_plus_get_items( $args ) {
		...
		elseif ( is_singular() ) {
			...
			if ( 'page' !== $wp_query->post->post_type ) {
				...
				if ( isset( $args["singular_{$wp_query->post->post_type}_taxonomy"] ) && is_taxonomy_hierarchical( $args["singular_{$wp_query->post->post_type}_taxonomy"] ) ) {
					$terms = wp_get_object_terms( $post_id, $args["singular_{$wp_query->post->post_type}_taxonomy"], array('orderby' => 'count') );
		...
		/* If viewing any type of archive. */
			...
			if ( is_category() || is_tag() || is_tax() ) {
				...
				if ( is_day() )
					$item['last'] = __( 'Archives for ', THEMENAME ) . get_the_time( MIPTHEME_DATE_DEFAULT );
	
				elseif ( is_month() )
					$item['last'] = __( 'Archives for ', THEMENAME ) . single_month_title( ' ', false );
	
				elseif ( is_year() )
					$item['last'] = __( 'Archives for ', THEMENAME ) . get_the_time( 'Y' );
			}
	
			else if ( is_author() )
				$item['last'] = __( 'Archives by: ', THEMENAME ) . get_the_author_meta( 'display_name', $wp_query->post->post_author );
		}
	
		/* If viewing search results. */
		else if ( is_search() )
			$item['last'] = __( 'Search results for "', THEMENAME ) . stripslashes( strip_tags( get_search_query() ) ) . '"';
	
		/* If viewing a 404 error page. */
		else if ( is_404() )
			$item['last'] = __( 'Page Not Found', THEMENAME );
 ...
 */
if ( ! function_exists( 'breadcrumbs_plus_get_bbpress_items' ) ) {
	function breadcrumbs_plus_get_bbpress_items( $args = array() ) {
		...
		elseif ( bbp_is_single_topic() ) {
			...
			if ( bbp_is_topic_split() )
				$item[] = __( 'Split', THEMENAME );
	
			elseif ( bbp_is_topic_merge() )
				$item[] = __( 'Merge', THEMENAME );
	
			elseif ( bbp_is_topic_edit() )
				$item[] = __( 'Edit', THEMENAME );
		}
 ...
 */
if ( ! function_exists( 'breadcrumbs_plus_get_parents' ) ) {
 ...
 */
if ( ! function_exists( 'breadcrumbs_plus_get_term_parents' ) ) {
 ...
 * @return string
 */
if ( ! function_exists( 'setup_breadcrumbs_plus' ) ) {
