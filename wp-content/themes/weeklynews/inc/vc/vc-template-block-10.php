...
if ( ! class_exists( 'MipTheme_VCExtendAddonClass_Block10' ) ) {

    class MipTheme_VCExtendAddonClass_Block10 {
        function __construct() {
            // We safely integrate with VC with this hook
            add_action( 'init', array( $this, 'integrateWithVC' ) );

            // Use this when creating a shortcode addon
            add_shortcode( 'mp_block_10', array( $this, 'renderMyBlock10' ) );
        ...
        public function integrateWithVC() {
            ...
            */
            vc_map( array(
                "name" => __("Block Layout 9", 'vc_extend'),
                "base" => "mp_block_10",
                "class" => "",
                "controls" => "full",
    ...
    */
        ...
        */
        public function renderMyBlock10( $atts, $content = null ) {
            ...
            $r = new WP_Query( apply_filters( 'block10_posts_args', $args ) );
            ...
            if ($r->have_posts()) :
                ...
                $ajax_data      = 'data-block="block-10" data-cat="'. $category_multiple_id .'" data-count="'. $post_limit .'" data-max-pages="'. ($r->max_num_pages + 1) .'" data-offset="'. $post_offset .'" data-tag="'. $post_tag_slug .'" data-sort="'. $post_sort .'" data-display="'. $category_display .'" data-img-format-1="'. $image_post_format_first .'" data-img-format-2="'. $image_post_format_second .'" data-img-width-1="'. $image_post_first_width .'" data-img-width-2="'. $image_post_second_width .'" data-img-height-1="'. $image_post_first_height .'" data-img-height-2="'. $image_post_second_height .'" data-text="'. $shorten_text_chars .'"';
                $ajax_block_id  = 'archive-page';

                $output .= '<section id="'. $ajax_block_id .'" '. $ajax_data .' class="module-timeline no-margin">'. ( ( ($section_title != '') || ( isset($post_paging) && ($post_paging == 'header_paging')  && ($r->max_num_pages > 1) ) ) ? '<header><h2>'. ( ( $section_link != '' ) ? '<a href="'. $section_link .'">'. $section_title .'</a>' : $section_title ) .'</h2><span class="borderline"></span>'. ( ( isset($post_paging) && ($post_paging == 'header_paging') && ($r->max_num_pages > 1) ) ? MipTheme_Ajax::setAjaxNav( $ajax_block_id, 'ajax-nav-header' ) : '' ) .'</header>' : '' );
    ...
    new MipTheme_VCExtendAddonClass_Block10();
