...
if ( ! class_exists( 'MipTheme_VCExtendAddonClass_Block5' ) ) {

    class MipTheme_VCExtendAddonClass_Block5 {
        function __construct() {
            // We safely integrate with VC with this hook
            add_action( 'init', array( $this, 'integrateWithVC' ) );

            // Use this when creating a shortcode addon
            add_shortcode( 'mp_block_5', array( $this, 'renderMyBlock5' ) );
        ...
        public function integrateWithVC() {
            ...
            */
            vc_map( array(
                "name" => __("Block Layout 8", 'vc_extend'),
                "base" => "mp_block_5",
                "class" => "",
                "controls" => "full",
    ...
    */
                    ...
                    array(
                        ...
                        "description" => "(e.g.: 75); Keep blank or '0' for default limit",
                        "holder" => "div",
                        "class" => ""
                    ),
        ...
        */
        public function renderMyBlock5( $atts, $content = null ) {

            global $post;

            $image_post_format_first    = 'post-thumb-7';
            $image_post_format_second   = '';
            $image_post_first_width     = '100';
            $image_post_first_height    = '80';
            ...
            $r = new WP_Query( apply_filters( 'block5_posts_args', $args ) );
            ...
            if ($r->have_posts()) :
                ...
                $ajax_data      = 'data-block="block-05" data-cat="'. $category_multiple_id .'" data-count="'. $post_limit .'" data-max-pages="'. ($r->max_num_pages + 1) .'" data-offset="'. $post_offset .'" data-tag="'. $post_tag_slug .'" data-sort="'. $post_sort .'" data-display="'. $category_display .'" data-img-format-1="'. $image_post_format_first .'" data-img-format-2="'. $image_post_format_second .'" data-img-width-1="'. $image_post_first_width .'" data-img-width-2="'. $image_post_second_width .'" data-img-height-1="'. $image_post_first_height .'" data-img-height-2="'. $image_post_second_height .'" data-text="'. $shorten_text_chars .'"';
                $ajax_block_id  = uniqid('mip-ajax-block-');

                $output = '<section class="section-'. $category_id .' news-layout clearfix" id="'. $ajax_block_id .'" '. $ajax_data .'>';
                if ( ($section_title != '') || ( isset($post_paging) && ($post_paging == 'header_paging') && ($r->max_num_pages > 1) ) ) {
                    $output .= '<header>
                                    <h2>'. ( ( $section_link != '' ) ? '<a href="'. $section_link .'">'. $section_title .'</a>' : ( ($section_title != '') ? $section_title : '<a href="'. get_category_link($category_id) .'">'. get_cat_name($category_id) .'</a>' ) ) .'</h2>
                                    <span class="borderline"></span>
                                    '. ( ( isset($post_paging) && ($post_paging == 'header_paging') && ($r->max_num_pages > 1) ) ? MipTheme_Ajax::setAjaxNav( $ajax_block_id, 'ajax-nav-header' ) : '' ) .'
                                </header>';
                } else if ( ($category_id) || ( isset($post_paging) && ($post_paging == 'header_paging') && ($r->max_num_pages > 1) ) ) {
                    $output .= '<header>
                                    <h2><a href="'. get_category_link($category_id) .'">'. get_cat_name($category_id) .'</a></h2>
                                    <span class="borderline"></span>
                                    '. ( ( isset($post_paging) && ($post_paging == 'header_paging') && ($r->max_num_pages > 1) ) ? MipTheme_Ajax::setAjaxNav( $ajax_block_id, 'ajax-nav-header' ) : '' ) .'
                                </header>';
                }
    ...
    new MipTheme_VCExtendAddonClass_Block5();
